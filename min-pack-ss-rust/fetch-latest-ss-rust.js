...
runKit(async (kit) => {
  ...
  const [ { name: RELEASE_NAME, assets: RELEASE_ASSET_LIST } ] = await (await fetchWithJump('https://api.github.com/repos/shadowsocks/shadowsocks-rust/releases?per_page=1', {
    headers: { 'x-github-api-version': '2022-11-28', 'accept': 'application/vnd.github+json', 'user-agent': 'dr-js/min-pack' }, jumpMax: 8
  })).json()
  ...
  for (const { name, browser_download_url: assetUrl } of RELEASE_ASSET_LIST) {
    ...
    const buffer = await (await fetchWithJump(assetUrl, { jumpMax: 8 })).buffer()
